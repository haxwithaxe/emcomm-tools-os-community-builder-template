...
* The file `config/package-lists/live.list.chroot` is generated by `lb config` it is intentionally not tracked in git.
...
## General Notes
* Symlinks don't work for things under `config` unless they are correct to the context in which they are deployed.
  * Symlinks relative to the same directory don't seem to work either.
